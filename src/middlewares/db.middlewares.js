...
export async function createDynamoRecord({dbName, item, messages = {} }){
    const params = new PutCommand({
        TableName: dbName,
        Item: item,
        ReturnValues:"ALL_OLD"
    })
    return docClient.send(params).then((response) => {
...
// async function scanDynamoRecords({ dbName, itemsArray = [], filters = [], params, segmentObject = {}, projectionExpression, operator = 'OR' }) {
...
//         const projection = (()=>{
//             if(typeof projectionExpression === 'object') return projectionExpression.map((proj ) => String(proj).trim()).join(', ').toString()
...
//         })
//     })()
//     const database = await docClient.send(scanParams).then((response) => {
//         return response
//     }).catch((error ) => {
...
// async function scanWorkers({ dbName, itemsArray = [], filters = [], params, workers = 5, projection, operator = 'OR' }) {
...
// async function getDynamoRecord({ dbName, keyObject, projection, filters = {}, messages = {} }){
...
// async function getDynamoRecordByParams({params,messages = {}}){
//     return docClient.send(new GetCommand(params)).then((response) => {
//         if(!response.Item) return { code: 404, body: messages[404] || ''}
//         return { code: 200, body: response.Item }
//     }).catch((error) => {
...
// async function queryDynamoRecord({ dbName, partition : { name, value }, sort, filters = [], messages = {}, projection = '', indexName = '' }){
...
// }
// async function updateDynamoRecord({ dbName, key, item, returnItem = false, errorMessages = {} }) {
...
// async function getBatchDynamoRecords({ itemArray, dbName,projection }){
...
// async function deleteDynamoRecord({ dbName, keyObject,returnItem=false, messages = {} }) {
...
//         ...(returnItem ? { ReturnValues: "ALL_OLD" } : {})
//     })
//     return await docClient.send(params).then((res) => {
